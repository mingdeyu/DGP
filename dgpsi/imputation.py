            ...
            if idx in linked_upper_kernels[0].exact_post_idx:
                if target_kernel.vecch:
                    if linked_upper_kernels[0].rep is not None:                     
                        ...
                        U_sp_latent, U_sp_obs_latent = U_matrix_sp(X[target_kernel.ord], target_kernel.imp_NNarray, target_kernel.scale[0], target_kernel.length, target_kernel.nugget[0], target_kernel.name, np.concatenate((invd, invd)), target_kernel.imp_pointer_row, target_kernel.imp_pointer_col)
